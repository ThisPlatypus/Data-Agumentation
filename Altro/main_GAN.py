...
def train_test_model(device , train_loader, val_loader,  N_EPOCHS, name, test_loader):
    ...
    #torch.save(model.state_dict(), f'MODELLI_final/{name}.pth')
...
#names = ['GAN_40', 'GAN_70', 'GAN_90']
#per = [0.4, 0.7, 0.9]

names = ['GAN_10', 'GAN_20', 'GAN_30','GAN_50', 'GAN_60', 'GAN_80']
per = [0.1, 0.2, 0.3, 0.5, 0.6, 0.8]

for i, name  in enumerate(names):
    #Data

    train = BalanceSetSynth(root_path_real=train_folder,root_path_fake=train_folder_GAN, transform=transform, N = sample_size_mine, rate_fake=per[i])
